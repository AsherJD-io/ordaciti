#!/usr/bin/env python3
"""
Enrich Project 255 data using curl with proper User-Agent header.
The fetchDetails endpoint works with a browser-like User-Agent.
The fetchProjects endpoint is blocked by Mod_Security (406).
We use the 251 project IDs already collected.
"""
import subprocess
import re
import json
...
print(f"Enriching {len(projects)} projects with proper User-Agent...")

for i, proj in enumerate(projects):
    pid = proj["project_id"]

    result = subprocess.run(
        ["curl", "-s", "--max-time", "8"] + HEADERS +
        [f"{BASE}?fetchDetails={pid}"],
    ...
    if not html or "Not Acceptable" in html:
        if i % 50 == 0:
            print(f"  {i+1}/{len(projects)}: ID {pid} - blocked")
        continue
    ...
    if (i + 1) % 50 == 0:
        ...
        print(f"  Progress: {i+1}/{len(projects)} (enriched so far: {enriched})")

    time.sleep(0.08)  # Rate limiting

# Final stats
data["enriched_at"] = "2026-09-18"
data["enrichment_method"] = "curl with User-Agent header via project255AJAX.php?fetchDetails=N"
...
print(f"\nSample (first 5 with LGA):")
count = 0
for p in projects:
    if p.get("lga") and count < 5:
        print(f"  ID {p['project_id']}: {p.get('title', 'N/A')}")
        print(f"    LGA: {p['lga']} | Sector: {p.get('sector')} | Status: {p.get('status')} | Amount: {p.get('contract_sum')}")
        count += 1
