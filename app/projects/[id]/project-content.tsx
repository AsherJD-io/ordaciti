    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <Card className="p-8 text-center">
          ...
          <p className="mt-1 text-muted-foreground">No evidence exists for project ID &ldquo;{id}&rdquo;.</p>
          <Link
            href="/projects"
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <Card className="mb-6">
          <div className="flex flex-col gap-4 p-1">
            <div className="flex items-start gap-4">
              <div className="mt-1 flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground font-bold text-lg font-mono">
                {id}
              </div>
              <div className="min-w-0">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                  ...
                  {normalized?.normalized_mda && (
                    <span>{normalized.normalized_mda}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            {normalized?.source_url && (
              <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 p-2.5">
                ...
                <span className="text-xs text-muted-foreground/70 truncate flex-1">{normalized.source_url}</span>
              </div>
            )}
          </div>
        </Card>
        ...
        {/* Recorded facts */}
        <SectionHeader title="Recorded facts" description="Information extracted from source data" />
        <Card className="mb-6">
          <CardContent>
        ...
        {signals.length > 0 && (
          <>
            <SectionHeader
              ...
              description={`${signals.length} signal${signals.length !== 1 ? 's' : ''} detected from evidence patterns`}
            />
                  ...
                  case 'REPEAT_INTERVENTION': {
                    ...
                    return (
                      <SignalCard
                        key={idx}
                        type={signal.type}
                        title="Repeat intervention pattern"
                      >
                        <div className="mb-3 flex items-center gap-2">
                          <Badge variant="warning">Cluster {d.cluster_id}</Badge>
                          <Badge variant="muted">{d.project_count} related project{d.project_count !== 1 ? 's' : ''}</Badge>
                        </div>
                        <div className="space-y-2 text-sm">
                          {d.first_project_date && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                              <span className="text-muted-foreground">First recorded</span>
                              <span className="text-foreground font-medium">{d.first_project_date}</span>
                            </div>
                          )}
                          {d.latest_project_date && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                              <span className="text-muted-foreground">Latest recorded</span>
                              <span className="text-foreground font-medium">{d.latest_project_date}</span>
                            </div>
                          )}
                          {intervals.length > 0 && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                              <span className="text-muted-foreground">Intervals</span>
                              <span className="text-foreground font-medium">{intervals.join(', ')} days</span>
                            </div>
                          )}
                          {d.cumulative_recorded_contract_value != null && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                              ...
                              <span className="text-foreground font-medium">
                                N{(d.cumulative_recorded_contract_value as number).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </span>
                            </div>
                          )}
                        </div>
                  ...
                  case 'CONTRACTOR_RECURRENCE': {
                    ...
                    return (
                      <SignalCard
                        key={idx}
                        type={signal.type}
                        title="Contractor recurrence"
                      >
                        <div className="mb-3 flex items-center gap-2">
                          <Badge variant="primary">{d.contractor}</Badge>
                          <Badge variant="muted">{d.project_count} project{d.project_count !== 1 ? 's' : ''}</Badge>
                        </div>
                        <div className="space-y-2 text-sm">
                          {d.first_project_date && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                          ...
                          {d.recorded_related_contract_value != null && (
                            <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                              ...
                              <span className="text-foreground font-medium">
                                N{(d.recorded_related_contract_value as number).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                          <span className="inline-flex items-center gap-1">
                            <span className="h-2 w-2 rounded-full bg-green-500" /> Available: {summary.available ?? 0}
                          </span>
                          <span className="inline-flex items-center gap-1">
                            <span className="h-2 w-2 rounded-full bg-amber-500" /> Partial: {summary.partial ?? 0}
                          </span>
                          <span className="inline-flex items-center gap-1">
                            <span className="h-2 w-2 rounded-full bg-red-500" /> Missing: {summary.missing ?? 0}
                          </span>
                          <span className="inline-flex items-center gap-1">
                            <span className="h-2 w-2 rounded-full bg-gray-400" /> Unknown: {summary.unknown ?? 0}
                          </span>
                        </div>
                      </SignalCard>
                    )
        ...
        {(cluster || allocation) && (
          <>
            <SectionHeader title="Context" description="Related project clusters and allocation patterns" />
            <Card className="mb-6">
              <CardContent>
                  ...
                  {cluster && (
                    <div>
                      ...
                      {cluster.history && cluster.history.length > 0 && (
                        <div className="mt-3">
                          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Cluster history</p>
                            ...
                            {cluster.history.map((h: { project_id: string; title: string; date_of_advert?: string | null; contract_amount?: number | null }, i: number) => (
                              <div key={i} className="rounded-lg bg-muted/30 px-3 py-2 text-sm">
                                <div className="flex items-center justify-between">
                                  <span className="font-medium text-foreground">#{h.project_id}: {h.title}</span>
                                  <Badge variant="muted">{h.project_id}</Badge>
                                </div>
                                <div className="mt-1 text-xs text-muted-foreground">
                                  {h.date_of_advert && <span>Advert: {h.date_of_advert}</span>}
                                  {h.contract_amount != null && <span className="ml-2">Value: N{h.contract_amount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>}
                                </div>
                              </div>
                            ))}
                          </div>
                    ...
                    <div>
                      ...
                      <div className="rounded-lg bg-muted/30 px-3 py-2">
                        <p className="text-sm text-muted-foreground">
                          ...
                          {' '}&middot; Recorded value: N{(allocation.recorded_contract_value as number).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </p>
                      </div>
        ...
        {project.sources.length > 0 && (
          <>
            <SectionHeader title="Sources & provenance" description="Where this information came from" />
            <Card>
              <CardContent>
                <ul className="space-y-2">
                    ...
                    if (src.url) {
                      return (
                        <li key={i} className="flex items-start gap-2 text-sm">
                          ...
                          <span className="ml-2 shrink-0 text-muted-foreground">({src.id})</span>
                        </li>
                      )
                    }
                    return (
